import PublicImage from '@/components/PublicImage'
import PublicVideo from '@/components/PublicVideo'

# Why do I need an SDK?

You may often want to integrate and add [Rebill](https://rebill.com/)'s payment solution directly and immediately into your website directly. This approach keeps and maintains your customers on your domain and on your website throughout their entire transaction process, ensuring and guaranteeing a consistent and branded experience for your customers' user experience.

This is exactly and precisely where Rebill's SDK comes into play and takes action here. Our SDK offers and provides a comprehensive and complete suite of tools that enables and allows you to use Rebill's payment experience without requiring or forcing your clients to navigate away or leave from your page or away from your website.

This integration method fully complies and adheres with PCI DSS standards and requirements standards, guaranteeing and ensuring the safety and security of customer data safety and protection. By integrating and implementing this solution, you can be confident and assured that customer information is kept secure and protected and safe and that your company complies and conforms with all regulations and rules governing the management and handling of payment information management and processing.

<PublicVideo
  filename="screenshots/sdk/checkout/videos/not-logged-checkout.mp4"
  alt="Rebill's SDK checkout."
  width="100%"
  height={'300px'}
  style={{ border: '1px solid #f0f0f0', borderRadius: '10px' }}
/>


## Our solution

Rebill’s SDK is a purely JavaScript-based solution that manages transactions and facilitates card tokenization for existing clients. This approach makes integrating with your app effortless: Simply insert a script tag into your HTML.

<CodeGroup>
  ```html
  <script src="https://sdk.rebill.com/v3/rebill.js"></script>
  ```
</CodeGroup>

## **Rebill Components**

Our SDK Checkout is composed of Rebill Components. Each component is a component-based UI solution that allows you to integrate the 100+ payment methods available easily. This feature is called **Rebill Components** and allows you to embed an iframe, delegating the full responsibility of transaction management to the provided UI.

By default, this approach renders a prebuilt checkout page to start accepting online payments, which is fully customizable ([see customization](https://docs.rebill.com/sdk/checkout#display)).


<PublicVideo
  filename="screenshots/sdk/checkout/videos/feature-one-click-checkout.mp4"
  alt="Rebill's SDK 1-click checkout functionality."
  width="100%"
  height={'300px'}
  style={{ border: '1px solid #f0f0f0', borderRadius: '10px' }}
/>

This prebuilt checkout page integrates by default Rebill's 1-click-checkout functionality. To further details [see user login](https://docs.rebill.com/sdk/checkout#user-login).
